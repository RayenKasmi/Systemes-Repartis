...
func main() {
	...
	log.Println("HO Consumer started")
	...
	go func() {
		for d := range msgs {
			...
			_, err := db.Exec("INSERT INTO data (source, name, value) VALUES (?, ?, ?)",
				data.Source, data.Name, data.Value)
			if err != nil {
				log.Printf("Error inserting data: %v", err)
			} else {
				log.Printf("Inserted data: [%s] %s = %s", data.Source, data.Name, data.Value)
			}
		}
	}()
